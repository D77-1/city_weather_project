...
const LEVELS = [
  { max: 50, className: 'level-good', icon: 'mdi:emoticon-excited-outline', title: '优', advice: '空气质量令人满意，适宜户外活动。', groups: ['适合运动', '可开窗通风'] },
  { max: 100, className: 'level-moderate', icon: 'mdi:emoticon-neutral-outline', title: '良', advice: '空气质量可接受，部分敏感人群需注意。', groups: ['敏感人群减少户外活动'] },
  { max: 150, className: 'level-unhealthy1', icon: 'mdi:emoticon-sad-outline', title: '轻度污染', advice: '老人、儿童应减少户外活动。', groups: ['佩戴口罩', '关闭门窗', '避免剧烈运动'] },
  ...
  { max: 300, className: 'level-very-unhealthy', icon: 'mdi:alert-octagon-outline', title: '重度污染', advice: '所有人群应尽量留在室内。', groups: ['避免外出', '佩戴防护口罩', '开启净化器', '不适请及时就医'] },
  { max: 999, className: 'level-hazardous', icon: 'mdi:skull-outline', title: '严重污染', advice: '空气质量极差，必须采取防护措施。', groups: ['禁止外出', '紧闭门窗', '建议转移', '不适立即就医'] },
]
...
.health-advice:hover { transform: translateX(4px); }
.advice-icon { flex-shrink: 0; display: flex; align-items: center; }
.advice-body { flex: 1; }
.advice-title { font-size: 14px; font-weight: 700; margin-bottom: 4px; display: flex; gap: 8px; align-items: center; }
.advice-text { font-size: 12px; line-height: 1.6; color: #5a5a5a; }
.future-summary { margin-top: 6px; font-size: 12px; color: #555; }
.risk-pill { font-size: 11px; background: rgba(255,255,255,0.75); padding: 2px 8px; border-radius: 999px; }
.advice-groups { display: flex; flex-wrap: wrap; gap: 6px; flex-shrink: 0; }
.group-tag { font-size: 11px; padding: 3px 10px; border-radius: 8px; background: rgba(255,255,255,0.7); white-space: nowrap; }
.advice-close { position: absolute; top: 8px; right: 8px; background: none; border: none; cursor: pointer; color: inherit; opacity: 0.5; padding: 4px; border-radius: 6px; display: flex; align-items: center; justify-content: center; }
...
.level-moderate .advice-title, .level-moderate .advice-icon { color: #b08350; }
.level-unhealthy1 { background: #fde8e0; border-color: #e07a5f; }
.level-unhealthy1 .advice-title, .level-unhealthy1 .advice-icon { color: #c0603f; }
.level-unhealthy2 { background: #fce4e4; border-color: #c1121f; }
.level-unhealthy2 .advice-title, .level-unhealthy2 .advice-icon { color: #c1121f; }
